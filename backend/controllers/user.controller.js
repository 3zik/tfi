...
export const followUnfollowUser = async(req,res) => {
    try {
        ...
        if (id === req.user._id.toString()) {
            return res.status(400).json({error: "you cant follow/unfollow yourself YOU IDIOT"});
        }
        ...
        if(isFollowing) {
            ...
            // no send notifs?  or do send HMMMM thining on this one.
        } else {
            //follow the user
            await User.findByIdAndUpdate(id, {$push: {followers: req.user._id}});
            await User.findByIdAndUpdate(req.user._id, {$push: {following: id}});
            // send notification to user that gets followed :)))
...
export const getSuggestedUsers = async(req, res) => {
    try {
        ...
        const users = await User.aggregate([
            ...
            {$sample:{size:10}} // FUNCTION LOADS TEN USERS, CHOOSES FOUR BY SELECTING 4 THAT THE USER DOES NOT ALREADY FOLLOW LOL
        ])
...
export const updateUser = async(req, res) => {
    ...
    try{
        ...
        // pwrd only null in the response, not saved to user.
        user.password = null;
